// ============================================
// LES ÉNIGMES DE LA CHOUETTE D'ATHÉNA
// Une porte de temple bloque le chemin : il faut résoudre une petite énigme
// (compter les pièces d'or, trouver l'intrus, continuer la suite, répéter la
// mélodie des cristaux). Jeu différent : on choisit avec les flèches et A.
// Après trois erreurs, la chouette montre la réponse : on n'est jamais bloqué.
// ============================================
...
namespace Puzzles {
    ...
    const KIND_COUNT = 0 // compter les pièces d'or
    ...
    // La porte du temple avec un point d'interrogation, surmontée d'un rideau
    // doré jusqu'au plafond : impossible de passer sans répondre
    export function gateImage(): Image {
        const im = image.create(20, 112)
        // rideau scintillant (assez dense pour qu'Ulysse le touche partout)
        for (let y = 0; y < 84; y++) {
            for (let x = 2; x < 18; x++) {
                if ((x + y) % 2 === 0)
                    im.setPixel(x, y, (x * 3 + y) % 7 === 0 ? 1 : 5)
            }
        }
        // la porte de marbre entre deux colonnes
        const top = 84
        im.fillRect(0, top + 6, 20, 22, 11)
        im.fillCircle(10, top + 8, 9, 11)
        im.fillRect(2, top + 8, 16, 20, 1)
        im.fillCircle(10, top + 9, 7, 1)
        im.fillRect(0, top + 6, 2, 22, 13)
        im.fillRect(18, top + 6, 2, 22, 13)
        im.fillRect(9, top + 12, 2, 16, 11)
        im.fillRect(3, top + 19, 14, 1, 11)
        im.print("?", 7, top + 10, 8, image.font8)
        return im
    }

    // ---------- Préparation des énigmes ----------
    function setupCount(level: number) {
        question = "Combien de pièces d'or ?"
        const n =
            level < 2
                ? randint(2, 4)
                : level < 4
    ...
    function draw(kind: number) {
        ...
        screen.print("La chouette d'Athéna", 22, 4, 1, image.font8)
        screen.printCenter(question, 18, 5, image.font8)

        if (kind === KIND_COUNT) {
            screen.fillRect(20, 27, 120, 40, 15)
            for (let i = 0; i < starsX.length; i++) {
                screen.drawTransparentImage(Assets.coin, starsX[i], starsY[i])
            }
