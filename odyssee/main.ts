// ============================================
// L'ODYSSÉE D'ULYSSE
// Un jeu de plateforme mythologique
// ============================================
...
namespace Game {
    ...
    // Pluie de pièces d'or (amphore) : des pièces jaillissent et retombent
    export function dropCoins(x: number, y: number) {
        for (let i = 0; i < 4; i++) {
            const s = sprites.create(Assets.coin, ItemKind)
            s.setPosition(x, y)
            s.vx = (i - 1.5) * 40
            s.vy = -120
            s.ay = 250
    ...
    function showTitle() {
        ...
        scene.setBackgroundImage(Assets.background(Assets.THEME_CYCLOPS))
        scene.centerCameraAt(80, 60)
        const boat = sprites.create(Assets.ship, DecoKind)
        boat.setPosition(92, 82)
        titleHero = sprites.create(Assets.ulysseR[0], DecoKind)
        titleHero.setPosition(42, 76)
        for (let i = 0; i < 4; i++) {
            const s = sprites.create(Assets.coin, DecoKind)
            s.setPosition(24 + i * 20, 60 + (i % 2) * 8)
        }
    }
    ...
    function spawnLevelObjects(theme: number) {
        for (const sp of Levels.spawns) {
            ...
            if (sp.kind === "P") {
                ...
                const s = sprites.create(Assets.lotusFlower, MushroomKind)
                s.x = x
                s.bottom = bottom
                s.z = 3
            } else if (sp.kind === "?") {
                ...
                const s = sprites.create(Assets.ship, PortalKind)
                s.x = x
                s.bottom = bottom
                s.z = 2
            } else if (sp.kind === "e") {
    ...
    // La porte de la chouette d'Athéna : une énigme pour l'ouvrir
    ...
    // ---------- Collisions ----------
    sprites.onOverlap(Player.Kind, ItemKind, (_p, coin) => {
        coin.destroy(effects.spray, 150)
    ...
    sprites.onOverlap(Player.Kind, Enemies.Kind, (p, enemy) => {
        if (state !== State.Playing && state !== State.Boss) return
        // La force d'Héraclès écrase tout ; sauter sur un ennemi l'élimine aussi
    ...
    sprites.onOverlap(Player.MagicKind, Enemies.Kind, (arrow, enemy) => {
        arrow.destroy()
        Enemies.kill(enemy)
    })

    sprites.onOverlap(Player.MagicKind, Bosses.Kind, (arrow, _boss) => {
        arrow.destroy(effects.spray, 100)
        Bosses.hit()
    })
    ...
    game.onUpdate(() => {
        ...
        if ((state === State.Title || state === State.Ending) && titleHero) {
            const baseY = state === State.Title ? 74 : 82
            titleHero.y = baseY + Math.sin(now() / 300) * 3
            titleHero.setImage(Assets.ulysseR[Math.idiv(now(), 200) % 2])
        }
    })
    ...
    game.onShade(() => {
        if (state === State.Title) {
            screen.fillRect(0, 14, 160, 44, 15)
            screen.printCenter("L'ODYSSÉE", 18, 5, bigFont)
            screen.printCenter("D'ULYSSE", 32, 5, bigFont)
            screen.printCenter("Le voyage vers Ithaque", 48, 1, image.font5)
