// ============================================
// ENNEMIS : sangliers, moutons-nuages, cochons ensorcelés, crabes,
//           statues vivantes, serpents de mer (marchent) ; mouettes,
//           harpies, corbeaux, poissons volants, aigles noirs,
//           nuages d'orage (volent)
// ============================================
...
namespace Enemies {
    ...
    function walkSpeed(theme: number): number {
        if (theme === Assets.THEME_CYCLOPS) return 22
        if (theme === Assets.THEME_AEOLUS) return 24
        if (theme === Assets.THEME_TITAN) return 26
        if (theme === Assets.THEME_STORM) return 30
        return 25
    }

    function flySpeed(theme: number): number {
        if (theme === Assets.THEME_STORM) return 1.5
        if (theme === Assets.THEME_SEA || theme === Assets.THEME_TITAN)
            return 1.3
        return 1.1
    }
